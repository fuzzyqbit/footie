...
- [ ] **Phase 1: Benchmark & Equivalence Harness** - Measurement instrument + golden-output safety net with committed baselines; lands first, nothing optimized ships without it
- [ ] **Phase 2: Data-Layer Cache & Batched Writes** - In-process id-indexed read cache over `db.py` + batched/atomic+durable writes that kill the O(n²) refresh rewrite
